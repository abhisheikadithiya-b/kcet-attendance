 ...
 * Proxies base64 webcam scans to Azure Face or Hugging Face Inference APIs securely.
 */
app.post('/api/external-verify', apiRateLimiter, async (req, res) => {
  ...
  // 1. Check Azure Face API credentials
  const azureKey = process.env.AZURE_FACE_KEY;
  const azureEndpoint = process.env.AZURE_FACE_ENDPOINT;
  if (azureKey && azureEndpoint) {
    console.log('Forwarding face scan to Microsoft Azure Cognitive Face Service...');
    try {
      // Remove base64 data header prefix if present
  ...
  // 2. Check Hugging Face Inference credentials
  const hfToken = process.env.HUGGING_FACE_TOKEN;
  if (hfToken) {
    console.log('Forwarding face scan to Hugging Face Inference serverless function...');
    try {
      const base64Data = image.replace(/^data:image\/\w+;base64,/, "");
      const buffer = Buffer.from(base64Data, 'base64');

      // Use a pre-trained face embedding model on Hugging Face (e.g. Facenet/InsightFace)
  ...
  // 3. Fallback: Notify client to run local SSD Mobilenet v1 + Face Recognition models
  return res.json({
    success: false,
    engine: 'Local Engine',
